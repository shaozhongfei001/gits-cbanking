...
## 5. 过强表述修正（依据 `GK-KE-OWNER-003` §8，2026-09-12 追加）

> 本决定由 TL 记录，**不覆盖历史 QA 报告**。本函原有表述按下列裁定修正，**不作废原函历史**。

| 本函原表述 | 裁定 | 修正后表述 |
|---|---|---|
| 「消费关系已获实证」（Q3 表） | **过强** | 仅支持**存在地图标识引用**（`KM-CORP-RM-PREVISIT` / `PRE_VISIT_PREPARATION`）；**不足以**证明配置加载与执行消费 |
| 将 `DENY_UNMAPPED_TASK` 归为「歧义处理」 | **不准确** | 它说明**没有匹配项时拒绝**；**不能证明**多个匹配项时如何处理。两种情况**需分别定义和验证** |
| KERT 中对象名零命中 → 判定「KERT 未认领」 | **证据不足** | 零命中**只说明该检索未命中**；不能证明不允许建设，也不能否定动态加载或外部配置的可能性 |
| 引用旧评审「应增加但缺失」 | **需限定** | 按其**日期和范围**作为**历史缺口**；不自动等同当前 HEAD 的全量运行结论 |
| 「主仓已确定性生成」（Q4 表） | **仍为 TL 陈述** | 需**实际实现位置及已有测试**支持；**不**升级为运行验收结论 |
| `KERT_GITS_STATE_MAPPING_CANDIDATE.md` 禁止误引 | **维持** | 属 WP1-3 产品推荐状态映射；保留 `CANDIDATE / FROZEN=NO / IMPLEMENTED=NO` 原始限定 |
| 「Owner 已选择等待 KERT 答复」 | **已过时** | `GK-KE-OWNER-003` 已在用户全项目维护者授权下作出 KERT 责任认领答复；**取消重复催签**，剩余按 §7 做技术落盘 |

## 6. 四问的最终处置（`GK-KE-OWNER-003` §2，替代本函 §1 的"待答"状态）

| 问题 | 决议 | 处置 |
|---|---|---|
| Q1 产品卡粒度 | `APPROVED_WITH_CONDITIONS` | 按**集合/集合入口**管理；固定选 `SIM-P001` 形成 `SIM-ASSET-P001`；**不得**把集合认作单卡 |
| Q2 路由认领 | `APPROVED_WITH_CONDITIONS` | **KERT 认领 `SIM-ROUTE-001`**；访前融资切片；无匹配与歧义**分别处理** |
| Q3 地图认领 | `APPROVED_WITH_CONDITIONS` | **KERT 认领 `SIM-MAP-FINANCE`**；限 SIM-C001/SIM-O01/INTERPRETATION |
| Q4 计划对象 | **APPROVED** | **KERT 接受** `ActivationPlan` 规划与运行集成；AC 是**编译前置约束**，禁作 planId |

**本函 §1 的四个"请回答"问题至此已由 Owner 决议答复完毕**，不再等待 KERT 逐条回复。
剩余为 §7.2 的技术落盘与核验。
