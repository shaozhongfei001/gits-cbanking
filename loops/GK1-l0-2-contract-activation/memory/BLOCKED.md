...
## 2. 唯一剩余阻塞：G2（来源与转换绑定）—— 已发出确认函，待 KERT 答复

> **Owner 已裁定方案 A（等待）**：向 KERT 维护方发出确认函 `docs/dispatch/GK-KE-L0-2-G2-KERT确认函.md`，答复后关闭 G2 → 激活。
> TL 已把 4 个问题从"未知"推进为"**有权威源依据的预判**"，详见下文与确认函。
